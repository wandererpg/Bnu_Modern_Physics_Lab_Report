...
实验三记录谱线左右边界的时间位置 $t_R$、$t_L$，同时记录一个自由光谱区的扫描周期 $T_{\mathrm{FSR}}$，并使用仪器给出的 $\nu_{\mathrm{FSR}}=\SI{2667}{\mega\hertz}$，由式\eqref{eq:bandwidth}计算出光带宽。
...
出光带宽的三次测量及频率换算结果见表\ref{tab:bandwidth-result}。其中 $t_R-t_L$ 为谱线左右边界的时间差。以第 1 次测量为例，由式\eqref{eq:bandwidth}得到
\begin{equation}
  \Delta\nu_{\mathrm{out},1}
  =\SI{2667}{\mega\hertz}\times
