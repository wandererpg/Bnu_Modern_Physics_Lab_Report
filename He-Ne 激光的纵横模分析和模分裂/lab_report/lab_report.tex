...
工作簿 A7:C20 中的非空数据给出了 $0^\circ$ 到 $180^\circ$、步长 $15^\circ$ 的 13 组测量。原始电位 $V_1$、$V_2$ 均为负，因此以 $U_1=-V_1$、$U_2=-V_2$ 表示峰值幅度，结果列于表\ref{tab:polarization-result}。
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[only marks,mark=*,mark size=1.5pt,blue] coordinates {
        ...
        (150,2.833) (165,2.633) (180,2.683)
      };
      \addlegendentry{$U_1$ 测量值}
      \addplot[blue,thick,domain=0:180,samples=181] {2.622+0.240*cos(2*(x+15.6))};
...
\begin{align}
  U_1(\theta) &= 2.622+0.240\cos[2(\theta+15.6^\circ)]\ \si{\volt},
  &&R^2=0.729,\\
  U_2(\theta) &= 2.983+0.636\cos[2(\theta-76.3^\circ)]\ \si{\volt},
  &&R^2=0.958.
\end{align}
两个拟合曲线的极大方向相差约 $91.9^\circ$，接近正交偏振所要求的 $90^\circ$。两个峰幅度的 Pearson 相关系数为 $r=-0.815$，说明一个峰增强时另一个峰总体减弱，也支持两分裂模具有近似正交的偏振方向。两峰幅度和的平均值为 \SI{5.58}{\volt}，样本标准差为 \SI{0.34}{\volt}；其并非严格恒定，主要反映激光功率漂移、峰位和峰高读数误差以及探测器响应变化。

从定量结果看，$U_1$ 拟合的 $R^2=0.729$、均方根残差为 \SI{0.119}{\volt}，明显差于 $U_2$ 的 $R^2=0.958$ 和 \SI{0.110}{\volt}；因此实验与理想余弦平方关系的偏差主要集中在第一个分裂峰，而不是两个峰的正交方向判断完全失效。偏差较大的首要原因是示波器读出的原始电位包含零线偏置、探测器暗电平和背景信号。严格来说，理论关系应作用于扣除背景后的峰值变化，测得电位更接近
\[
  U_i^{\mathrm{meas}}=U_{i,0}+k_i I_i(\theta),
\]
而本处理直接采用 $|V_i|$，会把 $U_{i,0}$ 和增益 $k_i$ 的影响混入偏振变化，尤其会使幅度较小、变化范围较窄的 $U_1$ 曲线看起来偏离余弦平方关系。其次，石英晶片的光轴、激光束和偏振片轴线未必完全对准，实际输出可能含有椭圆偏振或两个正交分量的串扰；偏振片消光比有限，也会造成本应接近零的峰仍保留较大读数。再次，两个谱峰的峰宽和间距有限，峰值光标的位置、峰形重叠以及扫描非线性会改变单次峰值读数。最后，测量过程中激光功率、PZT 扫描状态和探测器响应会漂移；两峰幅度和的相对波动约为 $0.34/5.58\approx6.1\%$，说明强度稳定性已经足以影响幅度拟合。

因此，$91.9^\circ$ 的极大方向差仍支持两个分裂模近似正交偏振，但幅度曲线不能按理想模型作高精度验证。改进时应先记录无光背景和示波器零线，用峰值相对基线的变化量代替 $|V_i|$；同时对每个角度重复测量并取平均，固定垂直档位和扫描参数，校准偏振片零点，并将非理想模型 $U_i^{\mathrm{meas}}=U_{i,0}+A_i\cos^2(\theta-\theta_i)$ 或其正弦形式直接用于拟合。
...
本实验依据共焦球面扫描干涉仪的比例标定方法，以 \SI{2667}{\mega\hertz} 自由光谱区处理三项实测数据。单管管长测量的两组纵模频差分别为 \SI{679.74}{\mega\hertz} 和 \SI{675.98}{\mega\hertz}，平均值为 \SI{677.86}{\mega\hertz}；反算管长为 \SI{221.3}{\milli\metre}，与尺量值 \SI{222}{\milli\metre} 的相对偏差为 \SI{0.32}{\percent}。
...
固定石英晶片、旋转偏振片测得的两个分裂峰幅度呈互补变化，余弦平方拟合的极大方向相差约 $91.9^\circ$，且幅度相关系数为 $r=-0.815$，说明两个分裂纵模近似正交偏振。由于两峰幅度和仍有 \SI{0.34}{\volt} 的样本波动，该结论应理解为在当前读数精度和光强稳定性下的定性到半定量判断。
...
\noindent\textbf{数据来源：}表 A1--A3 来自 \texttt{实验数据.xlsx}，记录管长测量和模式分析的原始峰位及逐差结果；表 A4 来自 \texttt{实验数据第二次.xlsx} 的 A1:F5，记录出光带宽测量的原始时间位置和频率换算量；表 A5 来自同一工作簿的 A7:C20，记录固定石英晶片、旋转偏振片时两个分裂峰的原始电位。教师签名栏仅作为版式预留。
...
\begin{table}[H]
  ...
  \begin{tabular}{ccccc}
    ...
    165 & -2.633 & -2.350 & 2.633 & 2.350 \\
    180 & -2.683 & -2.400 & 2.683 & 2.400 \\
    \bottomrule
  \end{tabular}
\end{table}
