...
共焦球面扫描干涉仪把微小频率差转换为随扫描电压变化的透射峰位置，是观察激光模谱的常用工具。本实验首先测量单根 He-Ne 激光管的相邻纵模频差并反算管长；随后通过螺旋测微器改变激光器工作状态，结合观察屏上的横向光斑和示波器上的峰列，辨认 $\mathrm{TEM}_{00}$ 与 $\mathrm{TEM}_{01}$ 模式并测量纵、横模频差；最后根据谱线左右边界的时间位置换算出光带宽，并在固定石英晶片条件下旋转偏振片，测量两个分裂谱峰的电位变化以判断其偏振关系\cite{guide}。
...
实验三将示波器设置为无限余辉（无限辉光时间），从较小值开始逐步增大压电陶瓷电源，使不同扫描状态下的谱线在屏幕上持续累积，形成可读取的宽带包络。选取其中一个宽带谱段，记录该谱段左右边界的时间位置 $t_R$、$t_L$；同时记录相邻重复峰之间的时间间隔 $T_{\mathrm{FSR}}$，并使用仪器给出的 $\nu_{\mathrm{FSR}}=\SI{2667}{\mega\hertz}$，由式\eqref{eq:bandwidth}计算出光带宽。图\ref{fig:bandwidth-scope}中标出了同一宽带谱段的 $t_L$、$t_R$、边界间隔 $\Delta t=t_R-t_L$，以及从一个峰到下一个相邻峰的 $T_{\mathrm{FSR}}$。

实验四先调节石英晶片使纵模分裂清晰，随后固定石英晶片和激光光路，在激光器输出镜与扫描干涉仪之间放置线偏振片。将偏振片刻度定义为 $0^\circ$，每隔 $15^\circ$ 旋转一次，记录两个分裂谱峰的电位 $V_1$、$V_2$，直至 $180^\circ$。由于线偏振片具有 $180^\circ$ 周期，$270^\circ$ 图像与 $90^\circ$ 状态具有等价的偏振片方向，可作为重复性和图像记录补充。测量过程中保持示波器垂直档位、扫描周期和光路不变。

实验五改变腔内石英晶片的转角示数 $\alpha$，记录示波器上所选左、右谱峰的时间位置。工作簿注明晶片垂直时示数为 $2.2^\circ$，故相对该位置的机械转角为 $\beta=\alpha-2.2^\circ$；作图保留原始示数 $\alpha$，不将其直接等同于晶体内部的光线与晶轴夹角。偏振态的观测则在调出足够分裂间距后固定晶片，把偏振片置于输出镜与扫描干涉仪之间，旋转偏振片比较两峰幅值。前者测量频差对晶片角度的变化，后者测量不同偏振分量的透射强弱。

\subsection{数据来源与处理方法}

晶片转角数据采用 \texttt{实验数据第二次.xlsx} 的 A23:D43：第23行为表头，第24--43行为20组记录；A列为角度示数，B、C列为左、右峰时间，D列为两者之差。按角度数值升序排列整行记录，采用 $T=\SI{6.3}{\milli\second}$、$\nu_{\mathrm{FSR}}=\SI{2667}{\mega\hertz}$ 换算频差。该组只绘制散点，不作拟合、插值或连线；低角度小峰间距记录保留，不在本报告中解释其成因。

管长和模式分析的四张原始示波器照片见 Fig.~\ref{fig:exp1-scope} 和 Fig.~\ref{fig:exp2-scope}，偏振关系测量的四张照片见 Fig.~\ref{fig:polarization-scope}。表格中的平均值与频率换算均按原工作簿公式复核。出光带宽数据取自 \texttt{实验数据第二次.xlsx} 的 A1:F5；偏振关系数据取自同一工作簿的 A7:C20，其中 A 列为偏振片角度，B、C 列为两个分裂峰的原始电位。偏振分析中将向下峰的原始电位取幅度 $U_i=|V_i|$，再进行余弦平方关系拟合。
...
将示波器设为无限余辉并从小到大调节压电陶瓷电源后，屏幕上保留了多次扫描形成的宽带谱线。图\ref{fig:bandwidth-scope}给出了该状态下的示波器记录及测量量标注：$t_L$ 和 $t_R$ 分别为同一个宽带谱段的左、右边界，$\Delta t$ 为该谱段两边界的时间差，$T_{\mathrm{FSR}}$ 为从一个谱峰到下一个相邻谱峰的时间间隔。出光带宽的三次测量及频率换算结果见表\ref{tab:bandwidth-result}。其中 $t_R-t_L$ 为单个宽带谱段的时间宽度。以第 1 次测量为例，由式\eqref{eq:bandwidth}得到

\begin{figure}[H]
  \centering
  \includegraphics[width=0.94\textwidth]{fig3.1_bandwidth_annotated.png}
  \bicaption{无限余辉下出光带宽测量的示波器记录及测量量标注。}{Oscilloscope record and measurement annotations for the output bandwidth under infinite persistence.}
...
工作簿 A7:C20 中的非空数据给出了 $0^\circ$ 到 $180^\circ$、步长 $15^\circ$ 的 13 组测量。原始电位 $V_1$、$V_2$ 均为负，因此以 $U_1=-V_1$、$U_2=-V_2$ 表示峰值幅度，结果列于表\ref{tab:polarization-result}。经核对实验记录，$165^\circ$ 和 $180^\circ$ 两项的 $U_1$ 存在抄录错误，已分别更正为 \SI{3.083}{\volt} 和 \SI{3.116}{\volt}；工作簿原文件保留不变。
...
\]
而本处理直接采用 $|V_i|$，会把 $U_{i,0}$ 和增益 $k_i$ 的影响混入偏振变化。其次，石英晶片的光轴、激光束和偏振片轴线未必完全对准，实际输出可能含有椭圆偏振或两个正交分量的串扰；偏振片消光比有限，也会造成本应接近零的峰仍保留较大读数。最后，测量过程中激光功率、PZT 扫描状态和探测器响应会漂移；两峰幅度和的相对波动约为 $0.25/5.65\approx4.4\%$，仍会影响幅度拟合。
...
\end{equation}
数值计算使用未舍入的 $2667/6.3$。例如 $\alpha=9.2^\circ$ 时，
\begin{equation}
 \Delta\nu_{\mathrm{obs}}
 =\SI{2667}{\mega\hertz}\,
 \frac{[-4.75-(-6.25)]\,\si{\milli\second}}{\SI{6.3}{\milli\second}}
 =\SI{635.0}{\mega\hertz}.
\end{equation}
...
\begin{figure}[H]
 \centering
 \includegraphics[width=0.96\linewidth]{figures/quartz_angle_scatter.pdf}
 \bicaption{石英晶片转角示数与两峰频差的实测散点。数据按角度升序整理；不连线、不拟合。}
 {Measured peak-frequency separation versus quartz-plate angle reading; scatter points only.}
 \label{fig:quartz-angle}
\end{figure}

\textbf{锯齿状走势。}关注主要频差分支，$2.8^\circ$--$3.0^\circ$ 的频差由 \SI{508.0}{\mega\hertz} 升至 \SI{677.3}{\mega\hertz}，在 $3.1^\circ$ 回落到 \SI{550.3}{\mega\hertz}；$6.2^\circ$--$9.2^\circ$ 从 \SI{529.2}{\mega\hertz} 总体升到 \SI{635.0}{\mega\hertz}，随后 $10.2^\circ$ 降至 \SI{317.5}{\mega\hertz}。$11.2^\circ$ 为 \SI{275.2}{\mega\hertz}，$12.2^\circ$ 回升为 \SI{317.5}{\mega\hertz}。这些分段升高、回落和再次升高构成了类似锯齿的非单调走势，其中 $9.2^\circ$--$10.2^\circ$ 的下降量为 \SI{317.5}{\mega\hertz}。中间若干测点变化较小，且角度采样并不均匀，因此不能从现有散点确定严格周期，也不能把每一段都认定为线性齿边；下降发生的精确角度只能限定在相邻采样点之间。
...
\end{equation}
其中 $\Delta\nu_{\mathrm{cav}}$ 是激光腔的纵模间隔，$m$ 是所选两峰的级次差。固定 $m$ 时频差沿一条分支变化；谱峰跨越、增益选择或模竞争改变可见峰和配对关系时，读数可能转入另一分支，表现为回落或折返。这为本组锯齿状走势提供了一个可能解释。由于记录只有每个角度的两个峰位，未连续标识峰的偏振身份和纵模序号，尚不能断定具体哪次回落就是模式跳变，也不能求出 $m$。这里的激光腔纵模间隔与扫描干涉仪的 \SI{2667}{\mega\hertz} 自由光谱区不同，后者仅用于标定，不能当作锯齿的重置幅度；扫描驱动电压本身呈锯齿波也不等同于频差随转角呈锯齿状。

在忽略旋光性等影响的双折射近似下，两个本征偏振方向对应普通光与非常光，彼此正交。外置偏振片旋转时，对这两个方向的投影强度分别按余弦平方与正弦平方变化，从而出现一峰增强、另一峰减弱的互补现象。实验四的幅值角扫描及约 $89.9^\circ$ 的极大方向差支持这一偏振关系；本组频差散点本身不单独确定偏振态。固定晶片时，理想外置检偏器主要改变透射幅值，而非腔内本征频率。

\textbf{误差与改进。}本组比例系数为 \SI{423.33}{\mega\hertz\per\milli\second}，因此峰间距读数若变化 \SI{0.05}{\milli\second}，换算频差将变化约 \SI{21.2}{\mega\hertz}；这只是灵敏度示例，不是已测得的仪器误差。较小的散点起伏可能含峰位判读、扫描非线性和晶片转动回差的贡献。建议在 $9.2^\circ$--$10.2^\circ$ 回落区加密角度采样，正反向重复测量；每次保存完整谱列，用检偏器跟踪同一偏振峰及其纵模级次，并在局部扫描段复核 $T$。只有同时确定峰的身份和重复性，才能区分真实分支切换与换峰、标定漂移造成的表观回落。低角度小峰间距的解释留待后续补充。
...
本实验依据共焦球面扫描干涉仪的比例标定方法，以 \SI{2667}{\mega\hertz} 自由光谱区处理各组峰间距与带宽数据。单管管长测量的两组纵模频差分别为 \SI{679.74}{\mega\hertz} 和 \SI{675.98}{\mega\hertz}，平均值为 \SI{677.86}{\mega\hertz}；反算管长为 \SI{221.3}{\milli\metre}，与尺量值 \SI{222}{\milli\metre} 的相对偏差为 \SI{0.32}{\percent}。
...
固定石英晶片、旋转偏振片测得的两个分裂峰幅度呈互补变化；更正 $165^\circ$ 和 $180^\circ$ 的 $U_1$ 抄录值后，余弦平方拟合的极大方向相差约 $89.9^\circ$，幅度相关系数为 $r=-0.940$，说明两个分裂纵模近似正交偏振。两峰幅度和的样本标准差为 \SI{0.25}{\volt}，该结论可作为当前读数精度下较可靠的半定量判断。

出光带宽三次测量的平均值为 \SI{1103.34}{\mega\hertz}，样本标准差为 \SI{43.89}{\mega\hertz}。后续测量应统一左右边界判据，采用正、反向扫描检验 PZT 滞回，并增加同一状态下的重复测量次数，以降低边界判读和扫描非线性造成的影响。

同时应同步保存观察屏图样、完整示波器波形和仪器设置，以降低峰位判读和模式跳变造成的影响。

晶片转角的20组记录经升序整理及 $T=\SI{6.3}{\milli\second}$ 标定后呈现分段升高与回落的锯齿状走势，其中 $9.2^\circ$--$10.2^\circ$ 的频差由 \SI{635.0}{\mega\hertz} 降至 \SI{317.5}{\mega\hertz}。双折射引起的谱线移动及所选纵模分支改变可作定性解释，但现有数据不足以确定严格周期或唯一的跳变机制；本组不作拟合，低角度小间距点不展开分析。
...
\noindent\textbf{数据来源：}表 A1--A3 来自 \texttt{实验数据.xlsx}，记录管长测量和模式分析的原始峰位及逐差结果；表 A4 来自 \texttt{实验数据第二次.xlsx} 的 A1:F5，记录出光带宽测量的原始时间位置和频率换算量；表 A5 来自同一工作簿的 A7:C20，记录固定石英晶片、旋转偏振片时两个分裂峰的原始电位，其中 $165^\circ$ 和 $180^\circ$ 的 $V_1$ 按实验记录完成抄录更正。教师签名栏仅作为版式预留。
...
\begin{table}[H]
 ...
 \end{tabular}
 \par\smallskip
 \parbox{0.94\linewidth}{\footnotesize 数据来源：\texttt{实验数据第二次.xlsx} 的 A23:D43。
 工作簿行号保留原始记录顺序供追溯；$\Delta t=C-B$，
 $\Delta\nu_{\mathrm{obs}}=2667\Delta t/6.3$。
 频差保留两位小数用于复核，不表示仪器测量精度；全部20组记录均保留。}
\end{table}
