...
共焦球面扫描干涉仪把微小频率差转换为随扫描电压变化的透射峰位置，是观察激光模谱的常用工具。本实验首先测量单根 He-Ne 激光管的相邻纵模频差并反算管长；随后通过螺旋测微器改变激光器工作状态，结合观察屏上的横向光斑和示波器上的峰列，辨认 $\mathrm{TEM}_{00}$ 与 $\mathrm{TEM}_{01}$ 模式并测量纵、横模频差；最后根据谱线左右边界的时间位置换算出光带宽\cite{guide}。
...
实验三将示波器设置为无限余辉（无限辉光时间），从较小值开始逐步增大压电陶瓷电源，使不同扫描状态下的谱线在屏幕上持续累积，形成可读取的宽带包络。记录包络左右边界的时间位置 $t_R$、$t_L$，同时记录一个自由光谱区的扫描周期 $T_{\mathrm{FSR}}$，并使用仪器给出的 $\nu_{\mathrm{FSR}}=\SI{2667}{\mega\hertz}$，由式\eqref{eq:bandwidth}计算出光带宽。图\ref{fig:bandwidth-scope}中标出了需要测量的 $t_L$、$t_R$、边界间隔 $\Delta t=t_R-t_L$ 以及相邻重复谱段的时间间隔 $T_{\mathrm{FSR}}$。

\subsection{数据来源与处理方法}

四张原始示波器照片见 Fig.~\ref{fig:exp1-scope} 和 Fig.~\ref{fig:exp2-scope}。表格中的平均值与频率换算均按原工作簿公式复核。出光带宽数据取自 \texttt{实验数据第二次.xlsx} 的 A1:F5，使用其中的 $t_R$、$t_L$、$T_{\mathrm{FSR}}$ 和 $\nu_{\mathrm{FSR}}$ 重新计算。
...
将示波器设为无限余辉并从小到大调节压电陶瓷电源后，屏幕上保留了多次扫描形成的宽带谱线。图\ref{fig:bandwidth-scope}给出了该状态下的示波器记录及测量量标注：$t_L$ 和 $t_R$ 分别为蓝色谱线包络的左、右边界，$\Delta t$ 为两边界的时间差，$T_{\mathrm{FSR}}$ 为相邻重复谱段的时间间隔。出光带宽的三次测量及频率换算结果见表\ref{tab:bandwidth-result}。其中 $t_R-t_L$ 为谱线左右边界的时间差。以第 1 次测量为例，由式\eqref{eq:bandwidth}得到
...
实验频率间隔和出光带宽均由示波器时间轴按自由光谱区比例换算，主要测量限制如下。
...
\noindent\textbf{数据来源：}表 A1--A3 来自 \texttt{实验数据.xlsx}，记录管长测量和模式分析的原始峰位及逐差结果；表 A4 来自 \texttt{实验数据第二次.xlsx} 的 A1:F5，记录出光带宽测量的原始时间位置和频率换算量。教师签名栏仅作为版式预留。
